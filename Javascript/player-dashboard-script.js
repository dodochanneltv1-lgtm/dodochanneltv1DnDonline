// Javascript/player-dashboard-script.js - โค้ดฉบับสมบูรณ์ (แก้ไขแล้ว)
...
/**
 * 🛠️ แก้ไขแล้ว: ปรับปรุงการแสดงผลศัตรูใน Panel และกล่องเลือกเป้าหมาย
 * เพื่อให้แน่ใจว่าศัตรูร่วม ('shared') แสดงผลอย่างถูกต้องและสามารถเลือกได้
 */
function displayEnemies(enemies = {}, currentUserUid) {
    ...
    // 1. กรองศัตรู: เอาเฉพาะศัตรูที่ตั้งเป้ามาที่คุณ (currentUserUid) หรือศัตรูร่วม ('shared')
    ...
    // 2. ล้างเนื้อหาเก่าออกก่อนแสดงผล
    container.innerHTML = '';
    ...
    // 4. แสดงผลศัตรูแต่ละตัว
    myEnemies.forEach(enemy => {
        const enemyKey = enemy.key;
        
        // A. อัปเดตส่วนแสดงผลหลัก (enemyPanelContainer)
        // เพิ่มรายละเอียดเล็กน้อยเพื่อให้เห็นภาพชัดเจน
        const targetText = enemy.targetUid === 'shared' ? 'ศัตรูร่วม' : 'เป้าหมายคุณ';
        container.innerHTML += `
            <div style="border: 1px solid #dc3545; padding: 5px; margin-bottom: 5px; border-radius: 3px; background-color: rgba(255, 0, 0, 0.1);">
                <strong>${enemy.name}</strong> (HP: ${enemy.hp}/${enemy.maxHp})
                <br>
                <small>เป้าหมาย: ${targetText}</small>
            </div>
        `;

        // B. อัปเดตกล่องเลือกเป้าหมายโจมตี (enemyTargetSelect)
        targetSelect.innerHTML += `<option value="${enemyKey}">${enemy.name} (HP: ${enemy.hp})</option>`;
    });
}
...
/**
 * 🛠️ แก้ไขแล้ว: ดึงข้อมูลศัตรูจาก allEnemiesInRoom โดยตรง
 */
async function performAttackRoll() {
    ...
    const roomId = sessionStorage.getItem('roomId');
    // 🔥 แก้ไข: ดึงข้อมูลศัตรูจากตัวแปร allEnemiesInRoom ที่ถูกอัปเดตแบบ Real-time
    const enemyData = allEnemiesInRoom[selectedEnemyKey];
    const playerData = allPlayersInRoom[uid]; // ใช้ตัวแปร Global allPlayersInRoom ได้เลย
...
/**
 * 🛠️ ยืนยันแล้ว: ใช้ enemyRef ชี้ไปยังโหนด enemies ที่ถูกต้อง
 */
async function performDamageRoll() {
    ...
    // 🔥 ยืนยัน: ชี้ไปที่โหนด enemies ที่ถูกต้อง
    const enemyRef = db.ref(`rooms/${roomId}/enemies/${selectedEnemyKey}`); 
    const enemySnapshot = await enemyRef.get();
    
    // ใช้ allPlayersInRoom แทนการดึงซ้ำ
    const playerData = allPlayersInRoom[uid];

    if (!enemySnapshot.exists() || !playerData) return;
...
firebase.auth().onAuthStateChanged(user => {
    if (user) {
        ...
        // 🔥 แก้ไข: อัปเดต allEnemiesInRoom และเรียก displayEnemies ทุกครั้งที่มีการเปลี่ยนแปลง
        db.ref(`rooms/${roomId}/enemies`).on('value', s => { 
            allEnemiesInRoom = s.val() || {}; 
            displayEnemies(s.val(), currentUserUid); 
        });
